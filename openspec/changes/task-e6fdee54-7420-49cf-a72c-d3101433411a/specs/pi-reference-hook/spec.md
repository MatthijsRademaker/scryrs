...
Defines requirements for the Pi reference trace hook — a transport-only TypeScript Pi extension that observes post-execution tool events, maps them to the canonical TraceEvent schema, and delegates ingestion to `scryrs record --stdin` via subprocess. This hook lives in `hooks/pi/` as reference source only; consumers install it into their own Pi extension locations.
