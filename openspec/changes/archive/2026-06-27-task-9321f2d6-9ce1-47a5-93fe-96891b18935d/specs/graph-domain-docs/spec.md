...
Defines the requirements for a dedicated domain-oriented graph documentation page in the project's internal developer documentation site (`.devagent/docs/docs/`). This page fills the "missing why" gap: current documentation describes graph implementation, architecture, and wire-contract details, but no single discoverable page explains what a scryrs graph represents in domain terms, what problem it solves, or how evidence becomes explainable routing context.
