...
Defines the live hotspot query endpoint (`GET /v1/repositories/{repository_id}/hotspots`) and SSE signal stream endpoint (`GET /v1/repositories/{repository_id}/signals`) on `scryrs server`. These read-only endpoints expose server-authoritative live hotspot state to dashboard and CLI consumers without scraping `.scryrs/hotspots.json` artifact files.
