...
Defines the requirements for a dedicated domain-oriented hotspot documentation page in the project's internal developer documentation site (`.devagent/docs/docs/`). This page fills the "missing why" gap: current documentation describes hotspot implementation, schema, and architecture, but no single discoverable page explains what a hotspot represents in domain terms, what user problem it solves, or how a developer should interpret and act on a `HotspotsReport`.
