...
Defines requirements for the cross-harness end-to-end verification entrypoint (`scripts/verify-trace-capture`) that proves reference hooks (Claude Code and Pi) correctly feed real `scryrs record --stdin` persistence without changing agent-visible behavior.
