...
Defines the versioned route-hint wire contract and the deterministic hint projection from route manifests. Route hints are downstream projections of `RouteManifestDocument` entries — they provide runtime consumers with explicit target, label, rank, evidence citations, and explanation without opaque ranking magic. This specification is a foundation contract; the `scryrs route explain` CLI command is deferred to a follow-up change.
