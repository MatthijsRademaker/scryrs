...
- [x] 1.1 Create `crates/scryrs-cli/tests/hotspot_e2e.rs` with:
...
- [x] 1.2 Add `#[test] fn e2e_record_to_hotspots_pipeline()` that:
...
- [x] 1.3 Add `#[test] fn e2e_empty_store_produces_success()` that:
...
- [x] 1.4 Add `#[test] fn e2e_missing_store_exits_2()` that:
...
- [x] 2.1 In `crates/scryrs-cli/src/lib.rs`, inside `mod hotspot_integration_tests`, add helper functions for each subject-bearing event family:
...
- [x] 2.2 Add `#[test] fn full_subject_family_fixture_produces_correct_ranking()` that:
...
- [x] 3.1 Run `cargo test -p scryrs-cli` to verify:
...
- [x] 3.2 Run `cargo insta review` to accept initial snapshots
- [x] 3.3 Run `cargo test -p scryrs-cli` a second time to confirm snapshots are stable (no drift on re-run)
- [x] 3.4 Verify that intentionally changing a hotspot output field (e.g., modifying a weight constant) causes the snapshot test to fail — confirming drift detection works

## 4. Cleanup and documentation

- [x] 4.1 Ensure no dead code, commented-out tests, or TODO comments remain
- [x] 4.2 Verify `cargo clippy -p scryrs-cli --all-features --tests` passes with no new warnings
- [x] 4.3 Verify `cargo fmt --check` passes on all changed files
