...
Defines the versioned hotspot report contract produced by `scryrs hotspots <PATH>` — the JSON envelope, ranked entry schema, deterministic scoring formula with tie-break rules, evidence references to SQLite event rows, CLI exit code semantics, and artifact file behavior.
