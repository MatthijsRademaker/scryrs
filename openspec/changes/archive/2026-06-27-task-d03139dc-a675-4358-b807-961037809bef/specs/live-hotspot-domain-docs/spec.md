...
Defines the documentation requirements for a new domain-first live hotspot page at `.devagent/docs/docs/live-hotspots.md`, plus the associated navigation update, cross-links, and stale/incorrect wording fixes in the project docs site. The page mirrors the successful `hotspots.md` domain-first pattern but covers the live server deployment mode: central ingest, shared live state, cumulative accumulators, live query API, and SSE signal streaming.
