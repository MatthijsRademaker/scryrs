...
The installer SHALL write hook files to consumer-side project-local directories relative to the current working directory. For Claude Code, the target is `.claude/hooks/scryrs-hook.mjs`. For Pi, the target is `.pi/extensions/pi-trace/index.ts`. The installer SHALL NOT write to user-global directories (e.g., `~/.pi/agent/extensions/`) in v1.

#### Scenario: Claude Code hook installed to .claude/hooks/

- **WHEN** `scryrs init --agent claude-code` is invoked from a project root
...
- **WHEN** `scryrs init --agent pi` is invoked from a project root
...
#### Scenario: No files written outside project root

- **WHEN** `scryrs init --agent <name>` is invoked
- **THEN** all files are written under the current working directory
- **AND** no files are written under `$HOME`, `/tmp`, or any absolute path outside CWD
...
The installer SHALL detect when the resolved target directory is the scryrs source checkout by checking for the presence of both a `Cargo.toml` referencing `scryrs-cli` in `[workspace.members]` AND a `hooks/claude-code/` directory in the CWD ancestry. When both markers are found, the installer SHALL exit 2 with a deterministic error explaining the reference-only boundary.

#### Scenario: Self-install detected and refused
...
- **WHEN** `scryrs init --agent claude-code` is invoked
- **THEN** the exit code is 2
- **AND** stderr explains that consumer config must not be written into the scryrs source repo
- **AND** no files are created or modified

#### Scenario: Self-install check walks parent directories
...
- **AND** self-install is detected and refused with exit 2
...
### Requirement: Consumer config is never written into the scryrs source repo

The installer SHALL preserve the reference-only boundary of the scryrs source repository. No consumer-side configuration files (`.claude/`, `.pi/extensions/`, `scryrs.json`, `.gitignore` modifications) SHALL be created or modified inside the scryrs source checkout by the installer.

#### Scenario: Scryrs source repo remains unchanged

- **GIVEN** the scryrs source repository after running the installer in an unrelated temp directory
- **WHEN** `git status` is checked in the scryrs source repo
- **THEN** no `.claude/`, `.pi/extensions/`, or consumer config files are present in the working tree
- **AND** the git working tree is clean of installer artifacts
