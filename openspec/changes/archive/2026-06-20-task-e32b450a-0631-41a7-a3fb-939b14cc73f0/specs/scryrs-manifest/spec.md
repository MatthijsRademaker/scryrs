...
Defines requirements for the `scryrs.json` hook manifest artifact — a versioned, machine-readable file at the repository root that describes supported harness hook definitions, the trace event schema version, the `scryrs record` invocation contract, reference artifact paths, and an explicit scope boundary excluding MCP/tool-catalog behavior.
