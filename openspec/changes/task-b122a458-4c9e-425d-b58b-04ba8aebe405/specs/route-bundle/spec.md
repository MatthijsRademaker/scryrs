...
Define a bounded, explainable context-loading plan derived from route hints. `scryrs route bundle` turns `.scryrs/routes.json` plus a query into a small JSON bundle that preserves route identity, load targets, reasons, and evidence while reusing the exact deterministic ranking behavior of `scryrs route explain`.
