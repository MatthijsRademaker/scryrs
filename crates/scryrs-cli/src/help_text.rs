...
COMMANDS\n\
  ...
  scryrs init --agent <NAME> [--mode live|local] [--ingest-url <URL>]\n\
      ...
      --mode live (default): remote ingest via scryrs server. Writes the committed\n\
        live config (ingest_url, workspace_id, docker_network) into scryrs.json remote\n\
        (the source of truth) and scaffolds .scryrs/compose.yml plus an overrides-only\n\
        .scryrs/.env. Each field resolves from flags, then env, then .scryrs/.env, then\n\
        scryrs.json remote; unresolved config fails fast. repository_id is derived from\n\
        the Git remote origin and agent_id is autogenerated per container — neither is\n\
        written to committed config.\n\
...
EXAMPLES\n\
  ...
  scryrs init --agent claude-code --mode live --ingest-url http://scryrs:8081 --workspace-id my-workspace --agent-id agent-1 --docker-network agent-net\n\
  scryrs up\n\
  scryrs dashboard\n\
