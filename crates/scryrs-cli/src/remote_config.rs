...
/// Remote config inputs resolved across the precedence chain before enforcing
/// which fields are mandatory for a specific command flow.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct ResolvedRemoteInputs {
    pub ingest_url: Option<String>,
    pub repository_id: Option<String>,
    pub workspace_id: Option<String>,
    pub agent_id: Option<String>,
...
impl ResolvedRemoteInputs {
    /// Convert partially-resolved inputs into the canonical live config.
    ///
    /// Returns `Ok(None)` when no ingest URL resolved, meaning live mode is not
    /// fully configured yet.
    pub(crate) fn into_resolved_remote(self) -> Result<Option<ResolvedRemote>, RemoteConfigError> {
        let Some(ingest_url) = self.ingest_url else {
            return Ok(None);
        };

        let repository_id = match self.repository_id {
            Some(value) => value,
            None => git_remote_origin_url().unwrap_or_default(),
        };
...
pub(crate) fn resolve_remote_config_with(
    ...
    resolve_remote_inputs(base_path, overrides).into_resolved_remote()
}

/// Resolve remote inputs across the full precedence chain without immediately
/// failing on missing required fields.
pub(crate) fn resolve_remote_inputs(
    base_path: Option<&Path>,
    overrides: &RemoteOverrides,
) -> ResolvedRemoteInputs {
    let manifest_remote = discover_manifest_remote(base_path);
    let dotenv = load_dotenv(base_path);

    ResolvedRemoteInputs {
        ingest_url: nonempty_value(resolve_field(
            overrides.ingest_url.as_deref(),
            ENV_INGEST_URL,
            &dotenv,
            &manifest_remote.ingest_url,
        )),
        repository_id: nonempty_value(resolve_field(
            overrides.repository_id.as_deref(),
            ENV_REPOSITORY_ID,
            &dotenv,
            &manifest_remote.repository_id,
        )),
        workspace_id: nonempty_value(resolve_field(
            overrides.workspace_id.as_deref(),
            ENV_WORKSPACE_ID,
            &dotenv,
            &manifest_remote.workspace_id,
        )),
        agent_id: nonempty_value(resolve_field(
            overrides.agent_id.as_deref(),
            ENV_AGENT_ID,
            &dotenv,
            &manifest_remote.agent_id,
        )),
        timeout_ms: resolve_timeout(overrides.timeout_ms, &dotenv, manifest_remote.timeout_ms),
    }
}
