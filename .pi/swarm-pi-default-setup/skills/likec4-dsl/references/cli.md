...
Note: This subcommand is provided for reference only. The repository workflow uses the LikeC4 CLI for validation (`likec4 validate`), not for querying architecture through the MCP service. Use the `likec4-dsl` skill and CLI for all architecture interactions.
