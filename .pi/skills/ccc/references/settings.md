...
**Runtime context:** the `envs.OPENAI_API_BASE` value in `global_settings.yml` must match your execution context — `http://127.0.0.1:8081/v1` for host, `http://host.docker.internal:8081/v1` for Docker containers. See the SKILL.md "Runtime Context" section for details.
...
**OpenAI-compatible (llama.cpp on bare metal, via LiteLLM):**
...
embedding:
  ...
  min_interval_ms: 5
envs:
  OPENAI_API_BASE: http://127.0.0.1:8081/v1   # host runtime; use host.docker.internal for Docker
