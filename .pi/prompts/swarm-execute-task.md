...
<skill_guidance>
<rule>Apply domain-specific rules for specialized code: workflow/taskflow/gate logic in src/manager/flowcontroller/ and src/shared/types/, dashboard UI design in src/dashboard/, and architecture context from .devagent/docs/. Use semantic code search to explore the codebase before editing.</rule>
<rule>Handle single-trivial-file changes and markdown-only edits directly.</rule>
<rule>For implementing logic, fixing bugs, or changing behavior: write a failing test first, confirm it fails for the right reason, implement the fix, then deliberately break the implementation and confirm the test catches it. Do NOT use test-first development for config changes, docs, or static content. When in doubt whether a change is behavioral, err on the side of writing tests.</rule>
</skill_guidance>
