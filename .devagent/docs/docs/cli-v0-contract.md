...
| `GET /api/accepted` | local | `200 OK` with JSON array of accepted knowledge list rows (`proposalId`, `title`, `targetType`, `reviewer`, `decidedAt`, `evidenceSummary`, `publishStatus`), sorted by `proposalId` ascending. `404 Not Found` when `.scryrs/accepted/` is missing. `502 Bad Gateway` when any accepted artifact is malformed, orphaned, or conflicting. |
| `GET /api/accepted/:proposalId` | local | `200 OK` with accepted knowledge detail (`acceptedContent` as primary, optional `originalProposedContent`, `reviewer`, `rationale`, `decidedAt`, `evidence`, `publishStatus`). `404 Not Found` when the proposal is not among accepted decisions. `502 Bad Gateway` when the accepted artifact is malformed or orphaned. |
| `GET /api/sessions`, `GET /api/sessions/:sessionId`, `GET /api/events`, `GET /api/proposals`, `GET /api/proposals/:proposalId`, `GET /api/accepted`, `GET /api/accepted/:proposalId` | live | `404 Not Found` with an explanation that the route is unavailable in live mode. |

**SPA contract:** The SPA is a Vue 3 application built with Vite, Bun, Tailwind CSS v4, and shadcn-vue, then embedded in the binary via `rust-embed`. Local mode shows Hotspots, Sessions, Proposals, Accepted, Events, and About. Live mode shows Hotspots, Signals, and About, hides Sessions/Events from navigation, and renders readable unavailable views on direct navigation to local-only routes. The Signals view owns reconnect behavior in the browser: it starts at `/api/signals?after=0`, tracks the last seen SSE id in memory, reconnects with `?after=<last_seen_id>`, and ignores replay duplicates on resume.
...
- Stdout: Docker Compose stdout plus deterministic success line.
- Stderr: Deterministic scaffold/network diagnostics on failure.

**Exit codes:**

- 0: Workspace-managed compose stack started successfully.
- 1: Docker invocation or Docker Compose runtime failure.
- 2: Missing `.scryrs/compose.yml`, unresolvable external network, missing external network, or unexpected arguments.
